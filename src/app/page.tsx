...
export default function TransactionDashboard() {
  ...
  useEffect(() => {
    async function fetchData() {
      const [custRes, pricingRes] = await Promise.all([
    ...
    <div className="min-h-[100dvh] bg-slate-50 flex flex-col font-sans text-slate-800">
      ...
      <main className="flex-1 flex flex-col xl:flex-row w-full max-w-[1700px] mx-auto p-4 lg:p-8 gap-6 xl:gap-10 items-stretch">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-[0_2px_10px_rgba(0,0,0,0.03)] flex flex-col overflow-hidden h-full">
            <form id="entry-form" onSubmit={handleSubmit} className="p-6 lg:p-8 flex-1 flex flex-col space-y-6 lg:space-y-8">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8">
                ...
                {/* Metric Inputs Row */}
                <div className="md:col-span-2 grid grid-cols-2 gap-4 lg:gap-6 mt-2">
        ...
        {/* =========================================================
            ...
            ========================================================= */}
        <div className="w-full xl:w-[45%] flex flex-col h-[600px] xl:h-auto min-h-0">
          <div className="mb-5 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <h2 className="text-lg lg:text-xl font-bold text-slate-800 tracking-tight">Recent Transactions</h2>

            <div className="flex items-center gap-2 bg-white px-3 py-1.5 rounded-lg border border-slate-200 shadow-sm">
              <span className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Fetch:</span>
              <select
                value={fetchLimit}
                onChange={(e) => {
                  ...
                  {paginatedTx.length === 0 ? (
                    ...
                    <tr key={tx.id} className="hover:bg-slate-50/80 transition-colors">
                      ...
                      <td className="px-5 py-4 text-right align-top">
                        <div className="font-black text-slate-800 text-sm">₹{Number(tx.net_payable ?? tx.total_price).toFixed(2)}</div>
                        ...
                        {tx.status && tx.status !== 'NORMAL' && (
                          <div className="mt-1 flex flex-col items-end gap-1">
                            ...
                            <div className="text-[10px] font-medium text-slate-500 bg-slate-50 border border-slate-200 px-2 py-1 rounded shadow-sm flex flex-col items-end leading-tight">

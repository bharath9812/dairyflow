...
import { deleteEmployee, createEmployee, resetEmployeePassword, updateEmployeeLocations } from './actions'
    ...
    <div className="max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-12 gap-6 items-stretch">
        ...
        <div className="sm:col-span-5 lg:col-span-4 flex flex-col gap-6 sticky top-24">
          <div className="bg-white/80 backdrop-blur-2xl border border-white/40 shadow-sm rounded-xl p-6 min-h-[420px] flex flex-col relative z-20">
            ...
            <form action={createEmployee} className="flex flex-col gap-5 flex-1">
              ...
              <div className="flex flex-col gap-1.5">
                <label className="text-xs font-bold text-slate-500 tracking-wider uppercase">Location Access</label>
                <div className="flex flex-col gap-2 max-h-32 overflow-y-auto bg-white/50 border border-slate-200 rounded-md p-3 text-sm font-medium">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input type="checkbox" name="location_all" value="true" className="w-4 h-4 text-onyx rounded border-slate-300 focus:ring-onyx" /> 
                    <span className="text-slate-700">All Locations (Full Access)</span>
                  </label>
                  <hr className="border-slate-100 my-1" />
                  {locations?.map(loc => (
                    <label key={loc.id} className="flex items-center gap-2 cursor-pointer">
                      <input type="checkbox" name="location_ids" value={loc.id} className="w-4 h-4 text-onyx rounded border-slate-300 focus:ring-onyx" /> 
                      <span className="text-slate-600">{loc.name} <span className="text-xs text-slate-400">({loc.short_code})</span></span>
                    </label>
                  ))}
                </div>
              </div>
        ...
        <div className="sm:col-span-7 lg:col-span-8">
          <div className="bg-white/80 backdrop-blur-2xl border border-white/40 shadow-sm rounded-xl p-0 flex flex-col min-h-[420px] max-h-[800px] overflow-hidden relative z-20">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                          ...
                          <div className="flex items-center gap-2 mt-1">
                            ...
                            {!isCurrent && (
                              <span className="text-[10px] text-slate-500 font-bold tracking-wide">
                                • Locs: {target.user_metadata?.allowed_locations === 'ALL' ? 'ALL' : (target.user_metadata?.allowed_locations?.length || 0)}
                              </span>
                            )}
                          </div>
                        </div>
                      ...
                      <div className={`flex items-center gap-2 self-start sm:self-auto shrink-0 ${isCurrent ? 'opacity-50 pointer-events-none grayscale' : ''}`}>
                        <div className="relative group/access">
                          <button type="button" className="px-3 py-1.5 text-xs font-semibold rounded-md border border-emerald-200 text-emerald-600 hover:bg-emerald-50 transition-colors flex items-center gap-1.5 h-8">
                            Access <svg className="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" /></svg>
                          </button>
                          <div className="absolute right-0 top-full mt-1 w-64 bg-white border border-slate-200 shadow-xl rounded-lg p-3 hidden group-hover/access:flex flex-col gap-2 z-50">
                             <form action={updateEmployeeLocations} className="flex flex-col gap-2">
                               <input type="hidden" name="uid" value={target.id} />
                               <label className="flex items-center gap-2 text-sm font-medium text-slate-700 cursor-pointer">
                                 <input type="checkbox" name="location_all" value="true" defaultChecked={target.user_metadata?.allowed_locations === 'ALL'} className="w-4 h-4 text-onyx rounded border-slate-300 focus:ring-onyx" />
                                 All Locations (Full Access)
                               </label>
                               <hr className="border-slate-100" />
                               <div className="max-h-40 overflow-y-auto flex flex-col gap-2 custom-scrollbar">
                                 {locations?.map(loc => (
                                   <label key={loc.id} className="flex items-center gap-2 text-sm font-medium text-slate-600 cursor-pointer">
                                     <input type="checkbox" name="location_ids" value={loc.id} defaultChecked={Array.isArray(target.user_metadata?.allowed_locations) && target.user_metadata.allowed_locations.includes(loc.id)} className="w-4 h-4 text-onyx rounded border-slate-300 focus:ring-onyx" />
                                     {loc.name} <span className="text-[10px] text-slate-400">({loc.short_code})</span>
                                   </label>
                                 ))}
                               </div>
                               <button type="submit" className="mt-2 w-full py-2 bg-emerald-500 text-white text-xs font-bold rounded-lg hover:bg-emerald-600 transition-colors shadow-sm">Save Access</button>
                             </form>
                          </div>
                        </div>

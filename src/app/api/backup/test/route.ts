...
export async function POST(request: Request) {
  try {
    ...
    const { email, tables, subjectPrefix, exportFormat, frequency, enabled } = body

    // Honour enabled toggle: if disabled, refuse execution
    if (enabled === false) {
      return NextResponse.json(
        { error: `${frequency} backup is currently disabled. Please turn it ON in settings and save before running.` },
        { status: 400 }
      )
    }
    ...
    // 1. Fetch transactions (Strict date range)
    ...
    // 2. Fetch Loans and Loan Payments if "all" scope selected (Strict date range)
    if (tables === 'all') {
      ...
      const { data: loanPayments, error: payError } = await supabase
        .from('loan_payments')
      ...
      if (loanError || payError) {
        return NextResponse.json({ error: loanError?.message || payError?.message }, { status: 500 })
      }
    ...
    // Build Lumina Terminal DESIGN.md Styled Email HTML (Premium Minimalist Look)
    const emailHTML = `
        ...
        <div style="max-width: 540px; margin: 0 auto; background-color: #ffffff; border: 1px solid #e2e2e4; border-radius: 12px; box-shadow: 0px 4px 20px rgba(0, 0, 0, 0.01); overflow: hidden;">
          ...
          <div style="padding: 32px;">
            ...
            <h2 style="font-size: 22px; font-weight: 600; color: #1a1c1d; letter-spacing: -0.02em; margin: 0 0 12px 0;">
              Manual Backup Report
            </h2>
            
            <p style="font-size: 13px; color: #4c4546; line-height: 1.6; margin: 0 0 24px 0;">
              This is a manual backup email triggered directly from the DairyFlow settings interface. The attached spreadsheet files are structured to support database restoration or analytical import.
            </p>
            ...
            <div style="background-color: #fcfcfd; border: 1px solid #eeeef0; border-radius: 8px; padding: 18px; margin-bottom: 24px;">
              ...
              <table style="width: 100%; border-collapse: collapse; font-size: 13px;">
                ...
                <tr style="border-bottom: 1px solid #eeeef0;">
                  ...
                  <td style="padding: 8px 0; text-align: right; color: #1a1c1d; font-weight: 600; text-transform: capitalize;">${frequency}</td>
                </tr>
                <tr style="border-bottom: 1px solid #eeeef0;">
                  <td style="padding: 8px 0; color: #7e7576; font-weight: 500;">Backup Scope</td>
                  <td style="padding: 8px 0; text-align: right; color: #1a1c1d; font-weight: 600; text-transform: capitalize;">${tables === 'all' ? 'All Tables' : 'Transactions Only'}</td>
                </tr>
                <tr style="border-bottom: 1px solid #eeeef0;">
            ...
            <div style="font-size: 11px; color: #7e7576; line-height: 1.5; margin-bottom: 32px; background-color: #fbfbfb; border-left: 3px solid #7e7576; padding: 10px 14px;">
              <strong>Security Warning:</strong> This backup contains sensitive financial records. Ensure this document is stored in an encrypted filesystem and is only visible to authorized administrators.
            </div>
    ...
    const resendRes = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        to: email,
        subject: `[Manual] ${subjectPrefix || 'Daily Backup'} - ${todayStr}`,
        html: emailHTML,
